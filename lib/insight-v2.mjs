 ...
 * Active prompt version: adaptive-v2.2
 * Working model (this feature branch): gpt-5.6-sol
 * Historical: adaptive-v2.1 (Mini vs Sol comparison); V1 cached shape remains supported.
 */

export const INSIGHT_V2_SCHEMA_VERSION = 2;
export const INSIGHT_V2_PROMPT_VERSION = "adaptive-v2.2";
...
export const INSIGHT_V2_PROMPT_VERSION_HISTORICAL = "adaptive-v2.1";
...
Before each Thread ask: if this disappeared, would the user lose a distinct observation? If no, omit it.
...
Focused → normally zero or one. Rich → only when several distinct connections are earned (1–3 max).
...
Expected: many Insights have no question. Focused → zero or one. Rich → zero, one, or at most two genuinely different angles.
...
rich — several connected details support more than one distinct Thread: notice 2–4 sentences; 1–3 Threads only if earned; 0–2 questions
